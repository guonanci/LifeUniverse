...
利用卡尔曼滤波，就可以把不同传感器的信息融合到一起。比方说，通过定位系统知道了汽车位置z1，但是GPS有误差，位置在一个范围内，还有个误差值方差σ1^2(西格玛)，z1只是表示可能的位置的平均值，或者说期望；而σ1^2代表了误差大小
...
总而言之，GPS定位告诉了位置信息，而其他传感器，雷达摄像头也能传达位置信息，如z2、σ2^2。传感器提供的标准差西格玛比较小，而GPS告诉我们的标准差σ1较大，那么如何把两个信息融合到一块呢？卡尔曼就说了，经过数学计算，有一个最优的估计

最优估计的位置z=σ2^2 z1/(σ1^2+σ2^2)+σ1^2 z2/(σ1^2+σ2^2). 简单分析下公式，σ2更小，对应的传感器更准确，计算权重时z2的权重应该更大些，你看z2前面乘以的是σ1^2，对上了吧。而结合新的方差σ^2满足，1/σ^2=1/σ1^2+1/σ2^2，就是卡尔曼最优结论

新的方差比原来两个方差都要小，所以最终会得出来一个新的估计情况。首先，它的均值介于两个估计值之间，而且它的方差比两个估计值都要小，或者说我们获得了一个更准确的估计。那么假如这辆汽车还有其他传感器，比方说速度计、加速度计，就尅继续进行卡尔曼滤波，获得一个更准确的结果

我们刚才介绍的这个模型，其实是最简单的一个卡尔曼滤波模型。实际情况下，卡尔曼滤波不仅是多维的，而且还跟时间有关，是一个时续的。卡尔曼滤波在航海上有啥应用呢？其实，大航海时代开始时，人类就已经不知不觉中使用这种滤波方法了

比方说，一个有经验的船长，在t1的时刻会有个位置，他还有个航向或者航速表，在t2时刻有个估计的位置，但是它并不会把这个位置作为最终自己的位置。他会用六分仪去测量，测量时又会有一个估计的位置，这两个估计的位置不一样，于是船长会用一个方法融合出一个新位置

现在我们知道了，最佳的融合方法就是卡尔曼滤波。有了t2的位置，还可以继续通过航向和航速，得出一个估计的位置，再通过六分仪，再得到一个估计的位置。如此，再把两个估计的位置拿去算，就会得到t3时刻的位置

多传感器融合的原理同样如此，每个传感器提取的数据，获得的特征，以及做出的判断，本质上都是一组带有误差的测量值。那么，我们可以通过卡尔曼滤波方法，融合起这些测量值，得到一个最准确的结果。宇宙飞船登月时，需要通过各种方法来判断自身位置，修正轨道

现在的自动驾驶汽车，也要通过各种传感器来给自己定位，同时判断周围环境，所使用的原理与早期的航海家是一样的，只不过，我们现在有了更加高精尖的设备、以及更好的数学理论。中国古人说，‘兼听则明，偏听则暗’，兼听就是多传感器融合，从兼听到明听需要的就是卡尔曼滤波

小鹏汽车NGP就是这样，通过多传感器融合与卡尔曼滤波技术，能够对自己的位置与周围的环境，做出更准确判断，从而让驾驶变得更安全和高效。小鹏汽车的NGP自动导航辅助驾驶功能，已经走过了百万公里路程，对小鹏汽车来说还只是一小步。但是在智能出行的长远道路上，这却是迈出的一大步

智能汽车将驶向何方？小鹏汽车还在持续探索。近日又听说，小鹏汽车在做NGP三千公里远程挑战，期待着他们挑战成功
