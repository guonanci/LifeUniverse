...
起初，这个自动机处于「初始状态」。随后，它顺序地读取字符串中的每一个字符，并根据当前状态和读入字符，按照某个事先约定好的「转移规则」，**从当前状态转移到下一个状态；当状态转移完成后，它就读取下一个字符。当字符串全部读取完毕后，如果自动机处于某个「接受状态**」，则判定该字符串「**被接受**」；否则，判定该字符串「**被拒绝**」。

注意：如果输入过程中某一步转移失败了，即**不存在对应的「转移规则**」，此时计算将提前中止。在这种情况下我们也判定该字符串「被拒绝」。
