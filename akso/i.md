...
- 后台管理系统：实现各个子页面修改表单或其他操作后，从其他页面再切换回本页面的缓存功能，注意事项：每个setup_script的name属性和route的name都采用parent_child下划线分隔命名，便于唯一化和定位
- 实现标签页的关闭所有、关闭左侧、关闭右侧、关闭其他功能
- 暂时不考虑标签页的‘固定’、‘取消固定’功能：删除路由配置子项的affix属性

v1.2.0新版本前端需要调节的地方：
1、首页、服务、套餐、商城这四个页面顶部的自定义bar实现各种机型适配；
2、从首页点击导航跳转到套餐页，如女士体检，跳转到套餐页后，分类要出现在最前面；
3、套餐和商城页向下滚动时，分类和排序筛选固定；
4、套餐页点击筛选时，弹层内容需要往下移动，避免遮挡；

- 增加后台管理系统的显隐列功能：有记忆特性，可记住该页面内之前勾选的列

@Arman 前端有以下几个需要做的：
1、客户端使用定位，获取当前所在城市及当前位置与医院门店的距离；
2、uniapp多环境配置，用于不同环境的验签秘钥不同；
3、物流信息查询，看看有没有相关的服务；
4、扫码自动识别卡号和卡密，便于用户绑卡操作；

配置vue文件格式化插件

除此之外，HBuild代码格式化你再试试其他插件

小程序的真机调试经常拿不到最新代码文件！！

客户端：使用逆解析定位城市，获取当前所在城市及当前位置与医院门店的距离；

uniapp区分生产环境和测试环境（体验版暂不确定，开发版和生产版本已区分）

客户端：拍照自动识别卡号和卡密，便于用户绑卡操作；

配置vue官方插件为默认的vue文件格式化插件；熟悉HBuilderX的格式化和保存相关的配置，方便代码编写

后台管理系统：图片列表上传组件增加拖拽排序功能

客户端：健康自测目前联调到热门推荐列表和全部测评列表，继续开发剩余的历史页面和问卷题目详情页

跟智康对接好接口后，再跟富文索要几张热门推荐的背景图。。。

针对体检这类低频、高决策成本的服务，推广需要聚焦长期价值、精准触达和客户生命周期管理。以下是系统化的推广策略，结合用户心理与行业特性设计：

### 一、重构产品价值——从「低频检测」升级为「健康管理入口」

1. **健康档案体系化**
   - 推出终身电子健康档案，每次体检数据自动更新并生成趋势分析报告
   - 提供异常指标5年对比功能，强化持续监测价值
   - 例：**美年健康**的「健康银行」概念，将单次体检转化为健康资产累积

2. **检后服务产品化**
   - 体检套餐绑定12次线上问诊/营养师咨询
   - 重大异常指标提供三甲医院绿色通道服务
   - 开发「体检+保险」组合产品，如检出特定疾病可获理赔

### 二、精准场景嵌入——制造「不得不检」的触发点

1. **人生阶段营销矩阵**

   | 生命周期阶段 | 定制套餐 | 触发场景 | 合作渠道 |
   |---|---|---|---|
   | 职场新人(22-25) | 基础职业病筛查 | 入职体检 | BOSS直聘/猎聘 |
   | 新婚夫妇 | 优生优育套餐 | 婚恋平台 | 婚礼纪/百合网 |
   | 中年危机(35+) | 癌症早筛套餐 | 重疾险购买 | 平安健康/众安保险 |

2. **环境线索植入**
   - 医院场景：在消化科候诊区投放「胃癌早筛套餐」广告
   - 写字楼场景：电梯广告展示「久坐族脊柱健康检测」
   - 社区场景：智能体脂秤+二维码，测量后推送定制化体检建议
...
1. **家庭健康积分体系**
   - 建立家庭账户，每位成员体检可累积健康币
   - 积分可兑换基因检测、疫苗预约等衍生服务
   - 设置「健康守护者」排行榜，年度积分前10%获VIP权益

2. **AI健康管家养成**
   - 用户上传历年报告训练专属AI健康助手
   - 通过完成体检任务解锁健康监测功能
   - 设置虚拟成就系统（如连续3年体检解锁「健康大师」称号）

### 四、B端生态构建——打造企业健康管理闭环

1. **HR SaaS增值模块**
   - 开发企业健康管理平台，自动分析员工体检数据
   - 生成《组织健康力白皮书》，提示用工风险
   - 对接EAP服务商，形成「体检-干预-咨询」链条

2. **供应链协同方案**
   - 与智能穿戴设备厂商数据互通（如华为手表异常心率预警触发体检推荐）
   - 药企合作：慢性病用药用户定向推送相关检测项目
   - 体检报告直接对接互联网医院复诊开药

### 五、记忆点营销——突破行业同质化

1. **感官体验升级**
   - 研发「无痛采血」「胶囊胃镜」等特色技术并注册商标
   - 设计体检中心气味管理系统（如采血区用柑橘香缓解紧张）
   - 开发VR导检系统，手机端实时查看各科室排队情况

2. **反常识传播**
   - 发布《20个你以为健康其实危险的指标》科普漫画
   - 制作「体检盲区」挑战视频，邀请医生解密常见漏检项目
   - 开展「你的体检报告能有多离谱」UGC征集活动

### 六、数据资产沉淀——构建精准复购模型

1. **客户健康风险分层**
   - 建立CLV（客户终身价值）预测模型，按风险等级划分：
   A类：三高人群（每6个月复购专项检测）
   B类：亚健康白领（年框套餐+季度微体检）
   C类：健康人群（2年1次基础体检+日常监测）

2. **智能唤醒系统**
   - 接入天气数据：雾霾预警时推送呼吸道专项检测
   - 结合新闻热点：某名人猝死事件后推送心脑血管套餐
   - 微信生态打通：当用户搜索「胃痛怎么办」时触发广告

### 关键执行要点

1. 建立健康数据中台，打通检前-检中-检后全链条
2. 培养健康管理师团队，转型为健康顾问而非销售
3. 与三级医院建立双向转诊系统，增强专业背书
4. 开发移动体检车等基础设施，突破场景限制

低频项目的本质是客户生命周期价值（LTV）的争夺，需通过服务延伸创造持续触点。当体检成为健康管理的核心枢纽时，低频属性自然被生态价值消解。

2024年02月24日-2024年02月28日技术部主要工作安排：
1、阿里云测试环境部署完成；
2、为申请知识产权，整理相关系统文档；
3、修复体检订单结算金额计算误差，测试中；
4、“阿克索健康服务平台”小程序发布v2.0.1修复版本；
5、后台增加页面列表字段筛选功能、图片拖拽排序；
6、健康评测开发联调中；
7、陪诊小程序需求整理、原型设计进行中；
8、“三八妇女节”活动页面设计中；

2024年03月03日-2024年03月07日技术部主要工作安排：
1、完成订单结算金额修复；
2、健康评测测试上线；
3、活动页配置开发及“三八妇女节”活动页面上线；
4、陪诊小程序原型及设计稿设计；
5、小程序医院品牌页及医院门店页页面设计；

一个关心我们发展的朋友提的建议:
围绕互联网三要素“拉新、激活、留存”

（1）针对高频业务，如何高效实现实现 线上和线下 引流激活？
（2）针对痛点业务，设计什么 爆品？
（3）核心口碑传播 实现的路径；

akso-client 测评的问卷题目页，还在调样式。  动态修改题目列表页的顶部标题。

questions第一个是否为提示类？

从develop分支建立feat分支，建议一切操作都在sourcetree上面进行！！
...
上线陪诊师小程序的操作指南功能；
阿克索健康网小程序：：健康评测开发/联调/测试/上线：首页（热门推荐、全部分类、历史测评）、详情页、问题列表页、报告结果页
后台管理系统：修复表格显隐列功能的单项undefined问题以及切换标签页后的显隐功能失效问题

*记得删除用不上的代码！！一定记得，图片必须上传到远程，不能放在小程序项目本地当中！！*
*要记得定义中间变量，提升代码可读性！！*
*更改pages.json/添加/未知错误发生后，要清空开发者工具胡全部缓存，点击HBuilderX下方命令行窗口内胡胡重启服务按钮*
*解决冲突时不确定的话，招当事人一起商量！*
*一定要认认真真自测！！*

# todo

页面高度为一屏幕，背景色为浅蓝；让智康配置和上传所有能操作的图片。

# 分享

回顾解决选项卡问题的思考及解决过程，包括缓存和关闭的一系列功能。
按周报、解决方式和路径，相关源码解读等方面来说；

# 样式问题

写样式挺费时间的，而且这次主要先完成功能，写js为主。
