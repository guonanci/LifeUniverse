...
整理外墙条幅；阅读ckeditor5编辑器的文档，有关协作编辑；新建修改家谱界面，和创建家谱分离。消息聊天的私聊界面增加默认头像的显示；动态详情可复制；动态详情样式调整；

聊天界面的顶部导航栏的用户昵称挪动到中间，类似微信；去除聊天界面的网络信号提示；阅读学习uni-app文档的editor、map组件等；
姓氏库增加搜索功能；市场业绩界面把“下级”改成“部门”；创建族谱、修改族谱时，堂号非填写；加好友增加一个等待提示‘等待好友验证’，删除家谱增加一个说明‘已经被激活成员的家谱不允许删除’，在家谱简介哪里增加一个激活人数；

去掉族谱信息界面的寻谱其他按钮；谱图缩放；
## 明日
APP3.0线上问题修复
