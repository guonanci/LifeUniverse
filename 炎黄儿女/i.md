...
notice-session-info 新的朋友；全家福qjfinfo.vue的数据来自上个页面，不调用接口；

梳理create_new_contact和createNewContact的所有逻辑，还有info.vue，WebSocket的所有逻辑，整理好代码，直接对接。

# 日报
## 今日
编写长连接消息查看、编辑用户资料和群资料接口相关的前端代码，简化设置，编写完后和子骏对接

参加周会，技术分享APP端的WebSocket代码、以及家谱录入后台，的代码逻辑、数据结构、和前端界面，和裕心对接原型图相关要点，后面继续做谱系图的排版预览、谱文编辑界面的技术调研工作

## 明日
和子骏对接长连接消息的接口和WebSocket消息类型，和浩哥对接家谱介绍模块的接口
