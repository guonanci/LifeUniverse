...
修复寻谱列表页列表项传承人跳转链接参数错误的问题；修复‘寻谱-族谱’页的发起人和家族姓氏显示错误的问题；修复‘寻谱列表’和‘寻谱-族谱’页的‘武氏氏’显示异常问题

测试zuyanhoutai项目自媒体管理模块；修复朋友圈发布列表头像不显示、图片集合不显示的问题；修复热门圈子列表圈子头像不显示的问题
## 明日
APP3.0线上问题修复和需求开发；APP端测试和修复；祖炎后台项目问题修复
