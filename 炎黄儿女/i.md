...
# 日报
## 今日
消息页点击搜索框时自动弹出键盘；加好友界面，修复addr_name为undefined时的页面判空问题，渲染恢复正常；
开发广场推荐页的新样式和交互

调整广场动态页的热门话题样式；调整视频页的样式，更换页面内图标，全屏显示，视频下方显示作者昵称和发布的内容；

因uni-app限制，更换底部导航栏的图标，从网络加载改回项目本地目录内的png；

以上更改尽量适配到APP端和微信小程序端，但是以APP端为主。

听取UI设计佩琪的意见，修改已开发的界面样式和交互：广场页title颜色调整；添加好友加号粗细调整；调整热门火堆图片的宽高；增加发布按钮的水波动效；动态的图片样式调整，增加图片宽度，增加间隙；视频页的头像白框变细些，下方进度条贴底边（设置tabbar的borderStyle为transparent）；视频页内，更换右上角拍照按钮为白底的圆款；加粗视频作者；消息页点击加号后，点击页面其他地方也能隐藏菜单；
## 明日
