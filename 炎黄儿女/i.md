...
collapse-icon的对齐样式以及children左边框的对齐样式；（添加配偶、孩子、和兄妹的操作直接放在右侧表单那）
...
# 日报
## 今日
开发修谱后台v1.0的谱系页，开发单击图标切换族人在世过世的功能，世代数目顶栏和下方谱系图同步变化；左侧菜单默认全部打开，顶部显示logo和公司名；

和子骏对接短视频、广场模块的接口，广场模块还差点赞功能，短视频模块对接完列表接口；
## 明日
和子骏对接短视频、广场模块的接口
