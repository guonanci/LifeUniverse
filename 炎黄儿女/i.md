...
改造用户或群聊信息chat-setting.vue，pick-user.vue界面前后端交互的前端代码，涉及删除好友、移除服务号会话、设置群聊备注等功能

后台的传承人图谱技术调研：改造传承人图谱样式，采用gojs方案，修改#myDiagramDiv元素的高度为800px；修改layout/default.html当中的.content元素的高度为850px，宽度为1700px；可实现同时显示‘最小点’、‘中心点’按钮，以及让树状图展示在尽量大点的空间；缩放效果挺好，不用改动相关代码（因为是水平宽度大的树状结构，所以需要在竖直方向上限定拖动距离和缩放倍数，以保证总体的拖动量和缩放量最小）；鼠标拖动平移时，拖动不到后代的问题已解决。

和裕心沟协调家谱录入后台的产品原型方面的编写；和子骏沟通后台里传承人家谱图的技术调研，了解到图谱的数据结构为扁平的数组，了解显示用到的字段。在JAVA版后台里后续采用canvas方案来实现，一个左右上下拖动更方便的传承人图谱
## 明日
开发APP端谱书界面
