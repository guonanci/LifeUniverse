...
短视频详情页的二级评论和帖子详情的二级评论（APP）；审核入口在后台配置、审核详情界面开发（有无审核结果）


修复家谱树的缩放按钮初始化时的样式问题和点击问题；修复家谱树界面底部加号点击不了的问题；添加朋友圈菜单入口，解决朋友圈第一次请求慢和重复请求两次的问题；

修复我的页面初始化时的undefined样式问题；开发传承人审核家谱人员资料编辑的功能；
## 明日
APP3.0线上问题修复
